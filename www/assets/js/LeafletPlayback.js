...
(function (factory) {
    ...
    initialize: function (startLatLng, options, feature) {
      ...
      if (options.labels) {
        if (this.bindLabel) {
          ...
          console.log(
            "Label binding requires leaflet-label (https://github.com/Leaflet/Leaflet.label)"
          );
        }
      }
    },
  ...
  L.Playback.Track = L.Class.extend({
    ...
    setMarker: function (timestamp, options) {
      ...
      if (lngLat) {
        ...
        this._marker = new L.Playback.MoveableMarker(
          latLng,
          options,
          this._geoJSON
        );
  ...
  L.Playback = L.Playback.Clock.extend({
    ...
    initialize: function (map, geoJSON, callback, options) {
      ...
      this._map = map;
      this._trackController = new L.Playback.TrackController(
        map,
        null,
        this.options
      );
      ...
      if (geoJSON instanceof Array) {
        ...
        if (geoJSON.type == "FeatureCollection") {
          for (var i = 0, len = geoJSON.features.length; i < len; i++) {
            this._trackController.addTrack(
              new L.Playback.Track(geoJSON.features[i], this.options),
              ms
            );
          }
        } else {
          this._trackController.addTrack(
            new L.Playback.Track(geoJSON, this.options),
            ms
          );
        }
      }
