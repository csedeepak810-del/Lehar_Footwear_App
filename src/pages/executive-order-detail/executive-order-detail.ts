...
export class ExecutiveOrderDetailPage {
    ...
    calculateAmountExecutive(qty,index,del,data:any)
    {
        ...
        this.orderDetail[index].amount = parseFloat(this.orderDetail[index].discounted_amount) +  this.orderDetail[index].gst_amount;
        this.orderDetail[index].amountAfterRoundOff = this.orderDetail[index].amount.toFixed();

        
        this.orderDetail[index].sub_total = parseFloat(this.orderDetail[index].sub_total).toFixed(2);
        this.orderDetail[index].discount_amount = parseFloat(this.orderDetail[index].discount_amount).toFixed(2);
        this.orderDetail[index].discounted_amount = parseFloat(this.orderDetail[index].discounted_amount).toFixed(2);
        this.orderDetail[index].gst_amount = parseFloat(this.orderDetail[index].gst_amount).toFixed(2);
        this.orderDetail[index].amount = parseFloat(this.orderDetail[index].amount).toFixed(2);
        ...
        { 
            ...
            this.order_data.order_total += parseFloat(this.orderDetail[i]['discounted_amount']);
        ...
        this.userDetail.order_grand_total = this.order_data.order_total + this.order_data.gst;
        ...
        this.userDetail.order_grand_total = (this.userDetail.order_grand_total).toFixed();
  ...
  update_order2(index,order_id,order_item_id,del:any)
  {
   ...
   this.update_order1(index,order_id,order_item_id,true)
    ...
    {  
        ...
        this.service.addData({"order_id":orderID},"cron/orderPdf").then((result)=>
        {
            ...
            if(result == 'success')
            {
                ...
                fileTransfer.download(url, this.file.externalApplicationStorageDirectory + '/Download/' + pdfName).then((entry) => {
    ...
    open_dispatch(){
        ...
        let dispatchOrder = this.modalCtrl.create(TargetAchievementPage, { 'type': 'DISPATCH', 'orderDetail': this.orderDetail, 'userDetail':this.userDetail});
        
        dispatchOrder.onDidDismiss(data => {
            this.events.publish('user:login');
        });
        dispatchOrder.present();
